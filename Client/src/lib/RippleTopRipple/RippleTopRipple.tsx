  ...
  public render(): JSX.Element {
      ...
      if (moreView === false && enterConfigedData.length > 15) {
        ...
        return (
          <React.Fragment>
            ...
            <br />
            <span className="ripple-more-view" onClick={this.handleShowMoreView}>
              더 보기
            </span>
          </React.Fragment>
        )
    ...
    const rippleAddMode = (topAddMode: boolean): JSX.Element => {
      if (topAddMode) {
        return (
          <React.Fragment>
            <button onClick={this.handleRippleAddMode} className="primary">
              답글 취소
            </button>
          </React.Fragment>
        )
    ...
    const rippleShowChildMode = (value: ShowChildRippleButtonFunction): JSX.Element | null => {
      ...
      if (value.topShowChildMode) {
        return (
          <button onClick={this.handleShowChildRipple} className="primary">
            답글 접기
          </button>
        )
      }
    ...
    const rippleChangeMode = (topChangeMode: boolean): JSX.Element => {
      if (topChangeMode) {
        return (
          <div className="ripple-flex">
            <button onClick={this.handleRippleChangeMode} className="info-half">
              수정 취소
            </button>
            <button onClick={this.handleRippleChangeMode} className="info-half">
              수정 확인
      ...
      return (
        <button onClick={this.handleRippleChangeMode} className="info">
          댓글 수정
        </button>
      )
    }
    ...
    const rippleDeleteMode = (topDeleteMode: boolean): JSX.Element => {
      if (topDeleteMode) {
        return (
          <div className="ripple-flex">
            <button onClick={this.handleRippleDeleteMode} className="danger-half">
              삭제 취소
            </button>
            <button onClick={this.handleRippleDeleteMode} className="danger-half">
              삭제 확인
      ...
      return (
        <button onClick={this.handleRippleDeleteMode} className="danger">
          삭제 하기
        </button>
      )
    }
